function [] = DLTdv3Brown(varargin)

% function [] = DLTdv3Brown()
%
...
% Renamed, Brown features added 11/11/08 L J Reiss
% Ty's 1/18/2009 changes incorporated, file prompts changed 2/2009 L J
% Reiss
% 23-June-09 Fixed header on intermarkerDistances.csv output file.
% 15-June-09 Added pause to allow "x" keypress to be processed on Macs.
%   See beginning of case 1.
%  added case statements for cases 8, 9.
% 06-Oct-09 XrayProject support for default directory added.
% 08-Dec-09 combined offsets and xypts into one xypts file.  Includes
% backward compatible support for old offset and xypts files.
% 10-Nov-22 use latest stand-alone calimread ( to prepare for matlab
% mmfileinfo replacing aviinfo in a new release)
% 2011-01-20 added support for input stack of tif or jpg
...
if nargin==0 % no inputs, just fix the path and run the gui
  ...
  if str2double(v(1:3))<7
    beep
    disp('DLTdv3Brown requires MATLAB version 7 or later')
    return
  end

  % check to see if we are in the path, add us if we are not
  dltloc=which('DLTdv3Brown');
  if ispc, p1=lower(path); else p1=path; end % current path
  % DLTdv3Brown directory
  dlttrim=dltloc(1:end-14); % 14 is # chars to strip off (12/08 change LJR)
  ...
  if inPath==0 % didn't find a match
    % add DLTdv3Brown's directory to the end of the path
    addpath(dlttrim,p1);
    disp(sprintf('Adding %s to the MATLAB search path',dlttrim))
  end
  call=99; % go create the GUI if we didn't get any input arguments
...
elseif nargin==1 % assume a switchyard call but no data
  ...
  h=uda.handles; % get complete handles of DLTdv3Brown GUI

  sp=get(h(32),'Value'); % get the selected point
  defaultdir = get(h(64),'String');
  xrayprojectdirhandle = h(65);
elseif nargin==2 % assume a switchyard call and data or XrayProject call
  call=varargin{1};
  if (call==99) % XrayProject call
      ...
      xrayprojectdirhandle = xrayprojhandles.defaultdir;
        else % switchyard call with data
    uda=varargin{2};
...
%% case 99 - GUI figure creation
  case {99} % Initialize the GUI
    ...
    disp(sprintf('\n'))
    disp('DLTdv3 (updated January 20, 2011 at Brown)')
    disp(sprintf('\n'))
    disp('Visit http://www.unc.edu/~thedrick/ for more information,')
    disp('tutorials, sample data & updates to this program.')
    disp(sprintf('\n'))
    disp('See XROMM wiki:')
    disp('https://wiki.brown.edu/confluence/display/ctx')
    disp('for features added at Brown University.')
    ...
    h(1) = figure('Units','characters',... % control figure
      ...
      'Name','DLTdv3Brown controls','NumberTitle','off',...
      'Position',[27 10 width top],'Resize','off',...
      'HandleVisibility','callback','Tag','figure1',...
      'UserData',[],'Visible','on','deletefcn','DLTdv3Brown(13)',...
      'interruptible','on');
    ...
    h(5) = uicontrol('Parent',h(1),'Units','characters',... % frame slider
      ...
      'DLTdv3Brown(1)','Enable','off');

    h(6) = uicontrol('Parent',h(1),... % initialize button
      'Units','characters','Position',[2.2 top-4.43 14 3],'String',...
      'Initialize','Callback','DLTdv3Brown(0)','ToolTipString', ...
      'Load the videos and DLT specification','Tag','pushbutton1');
                %-----position bottom changed from "top-2.73" to "top-4"
                % left changed from 21 to 19.
                % width changed from 14.4 to 18.4
                % 2008-Nov-19 LJReiss:
    h(7) = uicontrol('Parent',h(1),... % Save data button 
      'Units','characters','Position',[19 top-4 18.4 1.8],'String',...
      'Save Data','Tag','pushbutton3',...
      'Callback','DLTdv3Brown(5)','Enable','off');

    % h(8) - frame number text box (see below)
    
%--------------inserted 2008 Nov 18 by LJReiss----------------------------%
    % h(10) - Intermarker distance button 
  
    h(10) = uicontrol('Parent',h(1),...  % Intermarker distance button
      'Units','characters',... 
      'Position',[19 top - 2.2 18.4 1.8],'String', 'Check Distances',...
      'Tag','button_interDistance',...
      'Callback','DLTcalibration6X(5)',...
      'Enable','off', ...
      'Callback', 'DLTdv3Brown(17)');
%-------------------------------------------------------------------------%
    ...
    h(12)=uicontrol('Parent',h(1),... % Color display checkbox
      ...
      'colorVideos','enable','off','Callback','DLTdv3Brown(15)');
    ...
    h(13) = uicontrol('Parent',h(1),... % video gamma slider
      ...
      2,'Value',1,'Callback','DLTdv3Brown(1)', ...
      'Enable','off', 'ToolTipString','Video Gamma slider');
            % left changed from 39.8 to 40.4
            % width changed from 15.2 to 14.6
            % 2008-Nov-19 LJReiss:
    h(14) = uicontrol('Parent',h(1),'Units','characters',... % quit button
      'Position',[40.4 top-4.43 14.6 3],'String','Quit',...
      'Tag','pushbutton4','Callback','DLTdv3Brown(11)','enable','on');
    ...
    h(15) = uicontrol('Parent',h(1),... % camera 2 offset
      ...
      'Callback','DLTdv3Brown(8)','Enable','off');
    ...
    h(17) = uicontrol('Parent',h(1),... % camera 4 offset
      ...
      'Style','edit','Tag','edit3','Callback','DLTdv3Brown(8)', ...
      'enable','off');
    ...
    h(8) = uicontrol('Parent',h(1),... % frame text box
      ...
      'Callback','DLTdv3Brown(7)','enable','off');
            %-------------position bottom changed from top-5.23 to top-5.8
            % height changed from 1.7 to 1.8.
            % left changed from 21 to 20.
            % width changed from 14.4 to 16.4
            % 2008-Nov-19 LJReiss:
    h(22) = uicontrol('Parent',h(1),... % load previous data button
      'Units','characters','Position',[20 top-5.8 16.4 1.8],'String',...
    ...
    h(19) = uicontrol('Parent',h(1),... % add-a-point button
      ...
      'Add a point','Callback','DLTdv3Brown(12)','ToolTipString', ...
    ...
    h(35) = uicontrol(... % Autotrack mode pull-down menu
      'Parent',h(1),'Units','characters','Position',...
      [24 top-21.48 20 1.6],'String',...
      ['off|auto-retreat|auto-advance'...
      '|reverse semiautomatic|semiautomatic'...
      '|reverse automatic|automatic'...
      '|reverse multi|multi'],...
      'Style','popupmenu',...
      'Value',1,'Tag','popupmenu1','Enable','off','Callback',...
      'DLTdv3Brown(1)');
    ...
    h(53)=uicontrol('Parent',h(1),... % DLT visual feedback checkbox
      ...
      'DLTfeedbackbox','enable','off','Callback','DLTdv3Brown(1)');
    ...
    h(55)=uicontrol('Parent',h(1),... % Centroid finding checkbox
      ...
      'findCentroidBox','enable','off','Callback','DLTdv3Brown(14)');

    h(56) = uicontrol('Parent',h(1), ... % Autotrack predictor label
    ...
    % pull down menu widened 12/03/3008 LJR:
    h(57) = uicontrol(... % Autotrack predictor menu
      'Parent',h(1),'Units','characters','Position',...
      [27.2 top-23.63 26 1.6],'String',predMenu,'Style','popupmenu',...
    ...
    h(61) = uicontrol('Parent',h(1),... % DLT residual threshold
      ...
      'Callback','DLTdv3Brown(10)','enable','off');
   ...
   h(64) = uicontrol('Parent',h(1),... % default directory field
      'Units','characters','BackgroundColor',[1 1 1],'Position', ...
      [3.0 top-40.1 50.4 2.0],'String',defaultdir,'Style','text','Tag','defdir',...
      'enable','off', 'Visible', 'off');

   % note: defaultdir is saved above
  h(65) = xrayprojectdirhandle; % save pointer back to XrayProject figure
...
%% case 0 - Initialize button callback
  case {0} % Initialize button press from the GUI
      ...
      %-------------------------------inserted 11/11/08 LJReiss--------------%
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %added dedistortion (Baier 1/16/06) -- updated 11/17/08 LJReiss
      defaultdir = get(h(64),'String'); % if defaultdir was set in caller, it is here
      %     gdir = char(strcat(defaultdir, '/Grids/'));% initialize default directory (LJReiss 1/13/09)
      gdir = defaultdir;
      fc=-1;
      for i=1:uda.nvid
          if (~isequal(fc,0))
              % get the undistortion file
              [fc,pc]=uigetfile('*.mat',sprintf('Select Camera%d UNDTFORM File - Cancel if none exists',i),...
                  gdir); % default directory added (LJReiss 1/13/09)
              if isequal(fc,0)
                  disp(sprintf('No Undistortion loaded for Camera%d \n',i));
                  %changed to allow for viewing of distorted videos (Loomis 9/24/09)
                  break;
                  ...
                  gdir = pc;
 %                 dir_list = regexp(pc,'\\Grids|/Grids','split');
 %                 defaultdir = dir_list(1);

                  set(h(64),'String', gdir);%update default directory
                  if(h(65))
                      set(h(65),'String',gdir);
                  end
                  % change directory
                  %          cd(pc);
                  load([pc,fc]);
                  if exist('camd','var') == 1 % most UNDTFORM files are saved this way.
                      % [NOTE: if we remove flipud after calls to calimread and
                      % getNextFrame, we will need to reverse camud and camd.]  
                      eval(sprintf('uda.cam%dud=camud;',i));
                      eval(sprintf('uda.cam%dd=camd;',i));
                      eval(sprintf('uda.cam%dtform=undistortTform;',i));
                  else
                      % resets variables using old naming conventions from early
                      % versions of undistortion and saves a "fastload" version
                      % with the new naming conventions
                      if exist('BasePoints','var') == 0
                      ...
                      eval(sprintf('cam%dud=cp2tform(BasePointsflip,InputPointsflip,''lwm'');',i));
                      eval(sprintf('cam%dud.xlim=[min(InputPointsflip(:,1)),max(InputPointsflip(:,1))];',i));
                      eval(sprintf('cam%dud.ylim=[min(InputPointsflip(:,2)),max(InputPointsflip(:,2))];',i));
                      eval(sprintf('cam%dd=cp2tform(InputPointsflip,BasePointsflip,''lwm'');',i));
                      eval(sprintf('cam%dd.xlim=[min(BasePointsflip(:,1)),max(BasePointsflip(:,1))];',i));
                      eval(sprintf('cam%dd.ylim=[min(BasePointsflip(:,2)),max(BasePointsflip(:,2))];',i));
                      [pc, fc, ext, ] = fileparts([pc,fc]);
                      eval(sprintf('camud = cam%dud;',i));
                      eval(sprintf('camd = cam%dd;',i));
                      save([pc,filesep,fc,'fastload',ext], 'InputPoints', 'BasePoints',...
                          'BasePointsflip','InputPointsflip','camud','camd');
                      % [NOTE: if we remove flipud after calls to calimread and
                      % getNextFrame, we will need to reverse camud and camd.]  
                      eval(sprintf('uda.cam%dud=camud;',i));
                      eval(sprintf('uda.cam%dd=camd;',i));

                      clear InputPoint* BasePoint* camd camud dOffY dX dY fc1 ext undistortTform;
                  end

                  disp(sprintf('Loaded dedistortion transform matrix for camera%d.\n',i));
              end
          end
      end
      %end added dedistortion (Baier 1/16/06)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %---------------------------------------------------------------------%
    ...
    h(2) = figure('Units','characters',... % movie figure
      'Color',[0.831 0.815 0.784]','Doublebuffer','on', ...
      'KeyPressFcn','DLTdv3Brown(2)', ...
      'IntegerHandle','off','MenuBar','none',...
      'Name','DLTdv3Brown videos','NumberTitle','off',...
      'HandleVisibility','callback',...
      'UserData',uda,'Visible','on','Units','pixels', ...
      'DeleteFcn','DLTdv3Brown(13)','Pointer','cross', ...
      'Tag','VideoFigure','Interruptible','on');

    if uda.nvid==2 % two videos
      h(40)=axes('Position',[.01 .01 .49 .99],'XTickLabel','', ...
        'YTickLabel','','ButtonDownFcn','DLTdv3Brown(3)', ...
        'Interruptible','on', ...
    ...
    vdir = get(h(64),'String');
%    vdir = char(strcat(defaultdir, '/Videos/')); % initialize default directory (LJReiss 1/13/09)

    for i=1:uda.nvid
      % browse for each video file
      pause(0.01); % make sure that the uigetfile executed (MATLAB bug)
      [fname1,pname1] = uigetfile( ...
        {'*.avi;*.cin;*.cine;*.tif;*.jpg',...
        'All movie files (*.avi, *.cin, *.cine, *.tif, *.jpg)'; ...
        '*.avi','AVI movies (*.avi)'; ...
        '*.cin','Phantom movies (*.cin)'; ...
        '*.cine','Phantom movies (*.cine)';...
        '*.tif','TIF stack (*.tif)';...
        '*.jpg','JPG stack (*.jpg)'}, ...
        sprintf('Pick movie file %.0d',i), vdir); % default added 1/13/09
      ...
      vdir = pname1;
%      dir_list = regexp(pname1,'\\Videos|/Videos','split');
%      defaultdir = dir_list(1);
      set(h(64),'String',vdir); % update default directory
      if(h(65))
          set(h(65),'String',vdir);
      end
%      set(h(64),'String', defaultdir); % update default directory
...
%      mov=mediaRead(movname,1,false); % grab the 1st frame
      [im] = calimread(movname);  % grab the 1st frame
      im = flipud(im);
      % store the movie frame sizes
      uda.movsizes(i,1)=size(im,1);
      uda.movsizes(i,2)=size(im,2);

      % plot the frame
      redlakeplot(im(:,:),h(i+39));%>>>>flipud(im(:,:,1))???????
      uda.cdata{i}=im; % cache the image

      set(gca,'XTickLabel','');
      set(gca,'YTickLabel','');
      set(h(i+39),'Tag',tag);
      set(h(i+39),'Userdata',ud);
      set(get(h(i+39),'Children'),'ButtonDownFcn','DLTdv3Brown(3)');
    ...
    if uda.nvid>1
      ...
      switch dlt % setup DLT coefficients if desired
        case 'yes'
          dltdir = get(h(64),'String');  
%          dltdir = char(strcat(defaultdir(1),'/DLTcoeffs/')); % initialize default (added 1/13/09)
          pause(0.01); % make sure that the uigetfile executed (MATLAB bug)
          [fname1,pname1]=...
            uigetfile('*.csv','Pick the DLT coefficients file',dltdir);
          pause(0.01); % make sure that the uigetfile executed (MATLAB bug)
          if isequal(fname1,0)
              disp('DLT coeff file selection cancelled by user.');
              uda.dlt = 0;  % DLT off
          else
...
%              dir_list = regexp(pname1,'\\DLTcoeffs|/DLTcoeffs','split');
%              defaultdir = dir_list(1);
%              set(h(64),'String', defaultdir); % update default directory
              set(h(64),'String',pname1);
              if(h(65))
                  set(h(65),'String',pname1);
              end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Brown Check Distances button support (inserted 11/11/08 LJReiss)
    set(h(10),'enable','on'); % inter-marker distance button
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if exist('im2bw','file')==2 && exist('regionprops','file')==2 && ...
      ...
      set(h(55),'enable','on','Value',1); %changed value 2Feb2009 LJReiss
      disp('Detected Image Analysis toolbox, centroid localization is')
      disp('available, disable it via the checkbox in the Controls window.')
    else
...
pause(0.001); % allow event queue to be processed in case user presses "x".
      ...
      for i=1:uda.nvid
        ...
        if uda.drawVid(i)==true || get(h(63),'Value')==true
          ...
          if fr+offset <= 0 || fr+offset>frmax
            % no image available
            im=chex(1,round(xl(2)),round(yl(2)));
            uda.cdata{i}=im;  % cache the image
...
%              mov=mediaRead(tag,fr+offset,get(h(12),'Value'));
          ...
          hold(h(39+i),'off')
          if colorVal==0 %gray
            redlakeplot(im(:,:,1),h(i+39));
          elseif colorVal==1 && size(im,3)==1 % color w/ gray video
            redlakeplot(repmat(im(:,:,1)+(1-get(h(13),'Value') ...
              )*128,[1,1,3]),h(i+39));
        ...
        dltvf=get(h(53),'Value'); % 'DLT visual feedback' checkbox value

        % plot any DLT points (selected point) if desired
        if isnan(uda.dltpts(fr,1,sp))==0 && dltvf==1
          xy=dlt_inverse(uda.dltcoef(:,i),uda.dltpts(fr,:,sp)); % invdlt renamed
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          %added dedistortion (Baier 1/16/06)
          % distort the xy points for the camera
          eval(sprintf('xy=applyTform(uda.cam%sd,xy);',num2str(i)));
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          
        ...
        if isnan(uda.xypts(fr,i*2,sp)) && isnan(uda.dltpts(fr,1,sp)) ...
          ...
          %added dedistortion (Baier 1/16/06)
          if idx(1)==1
              uv=applyTform(uda.cam1ud,[u,v]);
              u=uv(1); v=uv(2);
          else
              uv=applyTform(uda.cam2ud,[u,v]);
              u=uv(1); v=uv(2);
          end
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          
...
%          xyPts(find(isnan(xyPts(:,1))==1),:)=[]; % changed 11/18/08 LJR
          xyPts((isnan(xyPts(:,1))==true),:)=[]; % changed LJR
...
%            xyzPts(find(isnan(xyzPts(:,1))==1),:)=[];
            xyzPts((isnan(xyzPts(:,1))==true),:)=[]; % changed LJR
            xyPts=dlt_inverse(uda.dltcoef(:,i),xyzPts); % invdlt renamed --LJR
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                     
            %added dedistortion (Baier 1/16/06)
            eval(sprintf('camD=uda.cam%sd;',num2str(i)))
            xyPts=applyTform(camD,xyPts);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
...
%% case 2 - key press callback
  case {2} % handle keypresses in the figure window - zoom & unzoom axes
    ...
    elseif cc=='x'
      uda.drawVid(:)=true;
      if get(h(35),'Value')<6 
        set(h(35),'Value',get(h(35),'Value')+2);
      else
        set(h(35),'Value',1); % set to 'off'
      end
      set(h(1),'Userdata',uda);
      return
    elseif cc=='X' % set to 'multi' or 'reverse multi'
      set(h(35),'Value',...
          8 + (get(h(35),'Value') - 2*floor(get(h(35),'Value')/2)));
      return
    ...
    elseif cc=='f' || cc=='b' || cc=='F' || cc=='B' && axh~=0 
      ...
      if cc=='f' && fr+1 <= smax
        if (get(h(35),'Value')==4 || get(h(35),'Value')==5 || ...
                get(h(35),'Value')==8 || get(h(35),'Value')==9) ...
...
%------------------------------------
%  inserted 2008 Dec 5 by LJReiss
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          %added dedistortion (Baier 1/16/06)
        % 'foo' changed to 'uda.xypts'.  Assignment to 'foo' removed.
        % LJReiss 2008-Dec-5

          foo=uda.xypts(fr,:,sp);
          foo(1,1:2)=applyTform(uda.cam1ud,foo(1,1:2));%uda.xypts(fr,1:2,sp)=applyTform(uda.cam1ud,uda.xypts(fr,1:2,sp));
          foo(1,3:4)=applyTform(uda.cam2ud,foo(1,3:4));%uda.xypts(fr,3:4,sp)=applyTform(uda.cam2ud,uda.xypts(fr,3:4,sp));
          [xyz,res]=dlt_reconstruct(uda.dltcoef,foo); % DLT reconstruction
  % above commented out 2008-Dec-5 LJReiss  dltout changed to [xyz,res]    
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%          
          %dltout=reconfu(uda.dltcoef,uda.xypts(fr,:,sp)); % DLT reconstruction

          
%------------------------------------
%LJR        [xyz,res]=dlt_reconstruct(uda.dltcoef,uda.xypts(fr,:,sp)); % DLT
    ...
    elseif cc==' ' % space bar (digitize a point)
      ...
      oData.cp=pixpos;
      oData.axn=vnum;
      DLTdv3Brown(3,uda,oData); % digitize a point
      return
    ...
    elseif cc=='R' % recompute 3D locations
      ...
      for i=1:size(uda.xypts,3)
        %-----------------------------------------------------------------%
        % inserted 2008-Nov-12 by LJReiss        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %added dedistortion (Baier 1/16/06)
        foo=uda.xypts(:,:,i); % 'foo' changed to 'uda.xypts'.  Assignment to 'foo' removed.
        % LJReiss 2008-Nov-12
        foo(:,1:2)=applyTform(uda.cam1ud,foo(:,1:2));
        foo(:,3:4)=applyTform(uda.cam2ud,foo(:,3:4));
%       uda.xypts(:,1:2,i)=applyTform(uda.cam1ud,uda.xypts(:,1:2,i));
%        uda.xypts(:,3:4,i)=applyTform(uda.cam2ud,uda.xypts(:,3:4,i));       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %-----------------------------------------------------------------%
%        rawResults=dlt_reconstruct(uda.dltcoef,uda.xypts(:,:,i));
        [rawResults,rawRes]=dlt_reconstruct(uda.dltcoef,foo);
        uda.dltpts(:,:,i)=rawResults(:,1:3);
        uda.dltres(:,:,i)=rawRes;
      end
    elseif cc=='c' % toggle "Find marker centroid" checkbox
        if get(h(55),'Value')==1
            set(h(55),'Value', 0);
        else
            set(h(55),'Value', 1);
...
%% case 3 - mouse click on image or spacebar callback
  case {3} % handle button clicks in axes
    ...
    if strcmp(seltype,'alt')==true || strcmp(seltype,'normal')==true
      ...
      if sum(isnan(uda.xypts(fr,:,sp))-1)<=-4 && uda.dlt==1
        ...
        % inserted 2008-Nov-12 by LJReiss
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %added dedistortion (Baier 1/16/16)
        foo=uda.xypts(fr,:,sp); %commented out 2008-Nov-12 by LJReiss
        foo(1,1:2)=applyTform(uda.cam1ud,foo(1,1:2)); %uda.xypts(fr,1:2,sp)=applyTform(uda.cam1ud,uda.xypts(fr,1:2,sp));
        foo(1,3:4)=applyTform(uda.cam2ud,foo(1,3:4)); %uda.xypts(fr,3:4,sp)=applyTform(uda.cam2ud,uda.xypts(fr,3:4,sp));
        [xyz,res]=dlt_reconstruct(uda.dltcoef,foo); % DLT reconstructionLJR 
        % above stmt commented out 2008-Nov-12 by LJReiss
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %-----------------------------------------------------------------%
        %[xyz,res]=dlt_reconstruct(uda.dltcoef,uda.xypts(fr,:,sp));LJR
    ...
    autoT=get(h(35),'Value'); % 1=off, 2=retreat 3=advance, 
    % 4=reverse-semi 5=semi, 6=reverse-auto 7=auto, 8=reverse-multi
    % 9=multi
    atDirection = 1; % forward tracking
    if autoT==(2*floor(autoT/2))
        atDirection = -1; % reverse tracking
    end
    if strcmp(seltype,'normal')==true && autoT>1
      if (autoT==2 || autoT==3) && fr<get(h(5),'Max'); % auto-advance
        set(h(5),'Value',fr+atDirection); % incr. or decr. current frame
        fr=fr+atDirection;
        % full redraw of the screen
        DLTdv3Brown(1,uda);
        % update the control / zoom window
        cp=uda.xypts(fr,axn*2-1:axn*2,sp);
        updateSmallPlot(h,axn,cp);
      elseif autoT>2 && autoT<8 % if rev-semi, semi, rev-auto, or auto
        keyadvance=0; % set keyadvance variable for DLTautotrack2
        DLTautotrack2fun(uda,h,keyadvance,axn,cp,fr,sp);
      end
    elseif strcmp(seltype,'extend')==true && (autoT==8 || autoT==9);%multi
...
%% case 5 - save data button
  case {5} % save data
      ...
      defaultdir = get(h(64),'String');
%      ddir = char(strcat(defaultdir(1), '/DigitizedData/'));
      ddir = defaultdir;
      %    pname=uigetdir(pwd,'Pick a directory to contain the output files');
      [fname,pname] = uiputfile(...
          '*.csv',sprintf('Enter filename prefix:'),...
          [ddir '']); % display the .csv files in folder, leave Filename blank
      pause(0.1); % make sure that the uigetdir executed (MATLAB bug)
      if isequal(fname,0)
          disp('Invalid file name or save cancelled by user.');
          return;
      end
      
      set(h(64),'String',pname); % update default directory
      if(h(65))
          set(h(65),'String',pname);
      end
      % get prefix
      sfix_list = ['(xyzpts.csv)|(xyzpts.csv)|(xyzres.csv)|(xypts.csv)|(offsets.csv)'...
          '|(markerDistances.csv)|(xyzCI.csv)|(xyzFilt.csv)|(.csv)'];
      pfix_list = regexp(fname,sfix_list,'split');
      pfix = char(pfix_list(1));

      if (strcmp((char(strcat(pfix, '.csv'))),fname))
          % only prefix was given
          newfname = char(strcat(pfix, 'xyzpts.csv'));
...
if(strcmp(overwrite,'Yes'))
    ...
    % create headers (xypts with offset)    
    numPts=size(uda.dltpts,3);
    offxyh=cell(uda.nvid*(1+numPts*2),1);
    
    %  (offset)
    for i=1:uda.nvid
      offxyh{i}=sprintf('cam%s_offset',num2str(i));
    end
    %  (xypts)
    for i=1:numPts
      for j=1:uda.nvid
        offxyh{(i-1)*uda.nvid*2+j*2+1}=...
          sprintf('pt%s_cam%s_X',num2str(i),num2str(j));
        offxyh{(i-1)*uda.nvid*2+(j*2)+2}=...
 ...
 fname=char(strcat(pfix, 'xyzpts.csv'));
	f1=fopen([pname,fname],'w');
      % header
...
%      f1=fopen([pname,filesep,pfix,'xypts.csv'],'w');
      fname1 = strrep(fname,'xyzpts.csv','xypts.csv');
      if(strcmp(fname,fname1)) fname1 = 'xypts.csv';end 
      f1=fopen([pname,fname1],'w');
      % header
      for i=1:numel(offxyh)-1
        fprintf(f1,'%s,',offxyh{i});
      end
      fprintf(f1,'%s\n',offxyh{end});
      % data
      for i=1:size(uda.xypts,1);
        tempDataOff=squeeze(uda.offset(i,:,:));
        for j=1:numel(tempDataOff)
             fprintf(f1,'%.6f,',tempDataOff(j));
        end       
...
%      f1=fopen([pname,filesep,pfix,'xyzres.csv'],'w');
      fname1 = strrep(fname,'xyzpts.csv','xyzres.csv');
      if(strcmp(fname,fname1)) fname1 = 'xyzres.csv';end 
      f1=fopen([pname,fname1],'w');
      % header
 ...
 %FEATURES COMMENTED OUT because we do not have toolbox for spline filter 
 if(1~=1) % ALWAYS SKIP OVER FEATURES
...
%          f1=fopen([pname,filesep,pfix,'xyzCI.csv'],'w');
          fname1 = strrep(fname,'xyzpts.csv','xyzCI.csv');
          if(strcmp(fname,fname1)) fname1 = 'xyzCI.csv';end 
          f1=fopen([pname,fname1],'w');
          % header
          for i=1:numel(CIh)-1
            fprintf(f1,'%s,',CIh{i});
          ...
          if exist('spaps','file')~=2
            disp(['DLTdv3Brown: The spline toolbox is not available so'...
...
%            f1=fopen([pname,filesep,pfix,'xyzFilt.csv'],'w');
            fname1 = strrep(fname,'xyzpts.csv','xyzFilt.csv');
            if(strcmp(fname,fname1)) fname1 = 'xyzFilt.csv';end 
            f1=fopen([pname,fname1],'w');
            % header
...
end  %END OF SKIPPED FEATURES

      % Save Inter-marker distances. ~~~~~~~~~~~~~~~~~~~~~~~~~~~
      numMarkers = size(uda.dltpts, 3);
      
      % Start by creating headers to enumerate marker pairs.
      count = 1;
      headers = cell(1,numMarkers*(numMarkers+1)/2);  % preallocate -- LJReiss
      for i = 1:numMarkers
          for j = i + 0:numMarkers
              headers{1,count} = sprintf('%1.0f_%1.0f', i, j);
              count = count + 1;
          end
      end
      
%      saveName = [pname, filesep, pfix, 'markerDistances.csv'];
      fname1 = strrep(fname,'xyzpts.csv','markerDistances.csv');
      if(strcmp(fname,fname1)) fname1 = 'markerDistances.csv';end 
      saveName = [pname, fname1];
      % Calculate the inter-marker distances.
      distances = CalculateIntermarkerDistances(uda.dltpts);
      % Save data out to file.
      SaveToCSVWithHeaders(saveName, distances, headers);
...
%    ddir = char(strcat(defaultdir(1), '/DigitizedData/')); 
    ...
    if isequal(fname1,0)
        disp('Load cancelled by user.');
        return;
    end
    
    set(h(64),'String',pname1); % update default directory
    if(h(65))
        set(h(65),'String',pname1);
    end
    ...
    % examine file header to see if file contains offsets (new format) or
    % only xypts (old format)
    fid=fopen([pfix,'xypts.csv'],'r');
    ctemp = textscan(fid,'%8c',1); 
    fclose(fid);
    
    % load the exported xy points
    tempData=dlmread([pfix,'xypts.csv'],',',1,0);
    len=size(tempData,1); % number of frames
    if(strcmp(ctemp,'pt1_cam1'))
        numpts=size(tempData,2)/(2*uda.nvid); % number of points
        uda.xypts=reshape(tempData,len,size(tempData,2)/numpts,numpts);
            % warn that file is old format.
            hmsg4 = msgbox(['WARNING - [prefix]xypts.csv file only contains xypts.  '...
                ,'New format files contain columns of frame offsets and xypts.  ',...
                'Offsets will be loaded from [prefix]offsets.csv if file exists.'], ...
                'Missing offsets in xypts.csv Warning','warn','modal');
            uiwait(hmsg4);
        
        % load the exported offset
        if (exist([pfix,'offsets.csv'],'file'))
            uda.offset=dlmread([pfix,'offsets.csv'],',',1,0);
        % check for non-zero offsets and pop up warning message if
        % there are non-zero offsets.
        for ivid = 1:uda.nvid
            ind = find(uda.offset(1:len,ivid)); % indices of non-zero offsets
            if(size(ind,1)~=0)
                % TODO: improve message.  This just reports the first
                % non-zero offset for each video.
                %  the same.  
                
                msgstr=sprintf('video %d has %d frame offset starting at frame %d.',...
                    ivid,uda.offset(ind(1)),ind(1));
                              
                hmsg5 = msgbox(msgstr, ...
                    'Non-zero Frame Offsets Warning','warn','modal');
                uiwait(hmsg5);
            end
        end
        else
            % warn that file is missing.
            hmsg3 = msgbox(['WARNING - offsets file [prefix]offsets.csv is missing. '...
                ,'All offsets are set to zero.'], ...
                'Missing offsets.csv Warning','warn','modal');
            uiwait(hmsg3);
            % setup the offsets arrays
            uda.offset(1:len,1:uda.nvid)=0;
        end
        
    else % file is new format with offset columns first, then xypts columns
        numpts=(size(tempData,2)-uda.nvid)/(2*uda.nvid);
        uda.offset=tempData(:,1:uda.nvid);
       % check for non-zero offsets and pop up warning message if
        % there are non-zero offsets.
        for ivid = 1:uda.nvid
            ind = find(uda.offset(1:len,ivid)); % indices of non-zero offsets
            if(size(ind,1)~=0)
                % TODO: improve message.  This just reports the first
                % non-zero offset for each video.
                %  Later, offsets are averaged and all are made
                %  the same.  
        ...
        uda.xypts=reshape(tempData(:,uda.nvid+1:end),len,...
            2*uda.nvid,numpts);
    ...
    if (exist([pfix,'xyzpts.csv'],'file')) &&...
            (exist([pfix,'xyzres.csv'],'file'))
        ...
        % reshape to uda.dltres size
        uda.dltres=reshape(tempData,len,1,numpts); 
        
    else
        button = questdlg(...
 ['WARNING - [prefix]xyzpts.csv file and/or ',...
            '[prefix]xyzres.csv file missing.  ',...
            '3D coordinates and residuals can be recalculated.'...
            'This process will take a couple minutes.'],...
            'Recalculate?',...
            'Continue', 'Cancel',...
            'Continue');
        if (strcmp(button, 'Continue'))
            lutBar = waitbar(0,'Recalculating 3D coordinates and residuals.');

        uda.dltpts(len,1:3,1:numpts)=NaN;
        uda.dltres(len,1,1:numpts)=NaN;
        
        for pt=1:numpts,
            for fr=1:len,
                % dedistortion
                foo = uda.xypts(fr,:,pt);
                foo(1,1:2) = applyTform(uda.cam1ud,foo(1,1:2));
                foo(1,3:4) = applyTform(uda.cam2ud,foo(1,3:4));
                % do DLT:
                [xyz,res] = dlt_reconstruct(uda.dltcoef,foo);
                % get the DLT points:
                uda.dltpts(fr,1:3,pt) = xyz(1:3);
                % get the DLT residual:
                uda.dltres(fr,1,pt) = res;
            end
            lutBar = waitbar(pt/numpts, lutBar);
        end
        
        close(lutBar);
        else
            disp(sprintf('\n Load cancelled by user. \n'));
            uda.xypts(1:len,1:2*uda.nvid)=NaN;
            uda.dltpts(1:len,1:3)=NaN;
            uda.dltres(1:len,1)=NaN;
            uda.offset(1:len,1:uda.nvid)=0;
            uda.numpts = 1; % reset to initial value
            numpts = 1;
        end        
    ...
    msgbox(['WARNING - check the offset values, they may not be correct'...
        ,' for partially digitized files.',...
        '  All offsets have been set to the average of the non-zero ',...
        'values in the loaded file.'], ...
        'Warning','warn','modal')
...
%% case 8 - validate video offsets text
    ...
    DLTdv3Brown(1,uda); % call self to update video frames
  ...
  case {13} % Window close via non-Matlab method
    ...
    if h(2)~=0 && uda.recentlysaved==0;
      ...
      if strcmp(savefirst,'yes')
        DLTdv3Brown(5,uda); % call self to save data
      else
...
%% case 15 - Color video checkbox
  case {15} % Click / unclick color video checkbox
    ...
    DLTdv3Brown(1,uda); % redraw screen

%% case 17 - Check Distances button---------------------------------------%
% inserted 2008-Nov-12 by LJReiss
    
  case{17} % Inter-Marker Distance callback
      
      if(exist('uda','var'))
        IntermarkerDistances(uda.dltpts);
      else
          disp('No data loaded!');
      end
%-------------------------------------------------------------------------%
...
% functions for the DLTdv3Brown.m
%
% Inputs:
...
atMode=get(h(35),'Value'); % auto-track mode: 1=off, 2= reverse, 3=advance,
% 4= reverse semi, 5=semi, 6= reverse auto, 7=auto, 8=reverse multi, 9=multi
trackFailed=false; % tracking loop pass / fail marker
atDirection = 1; % forward tracking
if atMode==(2*floor(atMode/2))
    atDirection = -1; % reverse tracking
end
% make sure we're not already at the end of the sequence
if atDirection==1 && fr==frmax, return, end
if atDirection==-1 && fr==1, return, end

% set all axes to draw off
uda.drawVid(:)=false;

% setup axn (axis number) matrix
if atMode==8 || atMode==9 % multi-mode
  AAxn=1:uda.nvid;
  ...
  if atMode==8 || atMode==9 % multi-mode
    ...
    for i=1:size(uda.xypts,3) % for each point
      if isnan(uda.xypts(fr,axn*2,i))==false && ...
          isnan(uda.xypts(fr+1*atDirection,axn*2,i))==true
        spM(spInc,A)=i;
...
while (fr+atDirection > 0) && (fr+atDirection < frmax + 1) &&...
        trackFailed==false && get(h(35),'Value')~=1

  fr=fr+atDirection; % increment or decrement the frame number by 1
...
%    mov=mediaRead(tag,fr+offset,get(h(12),'Value'));
    ...
    for S=1:numel(find(spM(:,A)>0));
      ...
      if ((atMode==8) || (atMode==9)) && isnan(uda.xypts(fr,axn*2,sp))==false
        ...
        [x,y,uda.predSeed]=AutoTrackPredictor(otherpts,fr-atDirection,get(h(57), ...
          'Value'),uda.predSeed);

        % constrain x & y to fit within imdat
        if x>size(imdat,2) || x<1
          x=round(otherpts(fr-atDirection,1));
        end
        if y>size(imdat,1) || y<1
          y=round(otherpts(fr-atDirection,2));
        end
  ...
  for sp=dltSet'
    if sum(isnan(uda.xypts(fr,:,sp))-1)<=-4 && uda.dlt==1 % 2+ xy pts
%------------------------inserted 2008-Nov-13 LJReiss---------------------%
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          %added dedistortion (Baier 1/16/06)
          foo=uda.xypts(fr,:,sp);%foo=uda.xypts(fr,:,sp); % commented out 2008-Nov-13 by LJReiss 18Dec08 put back
          foo(1,1:2)=applyTform(uda.cam1ud,foo(1,1:2));%uda.xypts(fr,1:2,sp)=applyTform(uda.cam1ud,uda.xypts(fr,1:2,sp));
          foo(1,3:4)=applyTform(uda.cam2ud,foo(1,3:4));%uda.xypts(fr,3:4,sp)=applyTform(uda.cam2ud,uda.xypts(fr,3:4,sp));
          %dltout=reconfu(uda.dltcoef,foo); % DLT reconstruction
          % above stmt commented out 2008-Nov-13 by LJReiss
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          % dltout=reconfu(uda.dltcoef,uda.xypts(fr,:,sp)); % DLT reconstruction

%-------------------------------------------------------------------------%
        
        [xyz,res]=dlt_reconstruct(uda.dltcoef,foo);%[xyz,res]=dlt_reconstruct(uda.dltcoef,uda.xypts(fr,:,sp)); % do DLT
      uda.dltpts(fr,1:3,sp)=xyz(1:3); % get the DLT points
      uda.dltres(fr,1,sp)=res; % get the DLT residual
  ...
  % advance or retreat a frame and redraw the screen
  set(h(5),'Value',fr);
  DLTdv3Brown(1,uda); % call self for video refresh

  if atMode==4 || atMode==5 || (atMode==8 && keyadvance==1) || (atMode==9 && keyadvance==1)
...
%-------------------------------inserted 2008-Nov-13 by LJReiss-----------%
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %added dedistortion (Baier 1/16/06)
      eval(sprintf('camD=uda.cam%sd;',num2str(i)))
      xy=applyTform(camD,xy);
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
 %------------------------------------------------------------------------%
...
%------------------------inserted 2008-Nov-13 by LJReiss------------------%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %added dedistortion (Baier 1/16/06)
    yptsN=(ypts(1):(ypts(2)-ypts(1))/100:ypts(2))';
    xptsN=(xpts(1):(xpts(2)-xpts(1))/100:xpts(2))';

    % copy the distort/undistort transform to a generic name
    eval(sprintf('camD=uda.cam%sd;',num2str(i)))

    % remove points outside the distort/undistort LWM region
    idx=find(xptsN<camD.xlim(1)*1.02 | xptsN>camD.xlim(2)*0.98);
    xptsN(idx)=[];
    yptsN(idx)=[];
    idx=find(yptsN<camD.ylim(1)*1.02 | yptsN>camD.ylim(2)*0.98);
    xptsN(idx)=[];
    yptsN(idx)=[];

    % distort the line of zero error
    xyptsN=applyTform(camD,[xptsN,yptsN]);

    % plot the line
	plot(h(i+39),xyptsN(:,1),xyptsN(:,2),'b-','HitTest','off');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-------------------------------------------------------------------------%
%----------line below commented out 08 Dec 2008 LJReiss
%    plot(h(i+39),xpts,ypts,'b-','HitTest','off');
  ...
  %--------------------inserted 2008 Nov 17 by LJReiss--------------------%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %added dedistortion (Baier 1/16/06)
      eval(sprintf('camD=uda.cam%sd;',num2str(i)))
      xyPts=applyTform(camD,xyPts);
...
% added dedistortion (Baier 11/16/06)
...
for i=1:N
  ...
  prevSys = predSys + (K*(e'))'; % a posteriori prediction of system state
...
% function [info]=mediaInfo(fname);
%   [note: only number of frames is used by caller. 2010 Dec 13 LJR]
% Wrapper which uses aviinfo or cineInfo depending on which is appropriate.

if strcmpi(fname(end-3:end),'.avi')
%  info=aviinfo(fname);
  info=get(mmreader(fname));
  info.NumFrames = info.NumberOfFrames;
elseif strcmpi(fname(end-3:end),'.cin')
  info=cineInfo(fname);
elseif strcmpi(fname(end-4:end),'.cine')
  info=cineInfo(fname);
elseif (strcmpi(fname(end-3:end),'.tif')||strcmpi(fname(end-3:end),'.jpg'))
    ...
    i1=strfind(fname(1:end-4),'.');
    if isempty(i1)
        info = imfinfo(fname);
        frCount = size(info,1);
        if(frCount>1)
         % single file stack of tif frames
            info=info(1);
            info.NumFrames = frCount;
        else % single tif frame
            info.NumFrames = 1;
        end
    else
        frameNo = sscanf(fname(i1+1:end-4),'%d',1);
        % find and count additional frames with same filename prefix.
        iCharsInFrameNo = length(fname)-4-i1;
        newFname = fname;
        nextFrameNo = frameNo;
        ...
        info.NumFrames = nextFrameNo - frameNo;
    end
else
    info=[];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% mediaRead
% ----------replaced by calimread and getNextFrame calls  Nov. 22, 2010 LJR
