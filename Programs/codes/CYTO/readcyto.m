function [out,status]=readcyto(pfile,number,cfield)
%function [out,status]=readcyto(pfile,number,cfield)
% READCYTO reads the hdf4-files of the CYTO-code.
%
% If cfield is not set, the program lists all fields stored in 
% the hdf file.
% For an automatic call of the program, the output field can
% be set by op_field: possible fields are:
%
% EX: [out,status]=readhdf('vineta10/cyto.37',1,'Potential');
