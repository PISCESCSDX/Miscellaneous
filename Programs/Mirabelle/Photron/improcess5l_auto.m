function improcess5l_auto(info,fastmode)
%==========================================================================
% function improcess5l_auto(info,fastmode)
%--------------------------------------------------------------------------
...
%average picture of the amount of 'info.fin{i}' picture files starting
...
%============================================
% Increase speed by clustering:
% amount of files to be clustered to calculate the overall average
clustersize = 16;
ncluster = floor(n/clustersize);
nrest = n-ncluster*clustersize;

%calcavg = input('Calculate the average picture now? y:1, n:0  ');
calcavg = 1;
...
if calcavg==1
aver = zeros(szx, szy);
% Average procedure (tested!)
% 1. First all clusters and then the rest
for i=0:ncluster-1
  before = zeros(szx, szy, ncluster);
  for j=1:clustersize
    ctr=ctr+1;
    num = i*clustersize+j+info.fi1{idir};
    numi(ctr) = num;
    disp_num(num,n+info.fi1{idir}-1);
    curfile = a(num).name;
    before(:,:,j) = double(imread(curfile));
  end
  % average over current cluster and previous average (weighted average!)
  aver = (sum(before,3)/clustersize + i*aver)/(i+1);
end
%
% 2. Now the rest of n
before = zeros(szx, szy, nrest);
n_now = ncluster*clustersize;
for j=1:nrest
  ctr = ctr+1;
  num = n_now +j+info.fi1{idir};
  numi(ctr) = num;
  disp_num(num,n+info.fi1{idir});
  curfile = a(num).name;
  before(:,:,j) = double(imread(curfile));
end
% average over last cluster and previous average (weighted average!)
aver = (sum(before,3) + n_now*aver)/(n_now+nrest);
% convert to double
aver = double(aver);
% write averaged picture
fn = 'avg.tif';
...
% Substract the average picture to obtain fluctuation data
%==========================================================================
...
if info.den{idir} == 1
  ...
  wdenoise4l_auto(subinfo,fastmode);
end
