function [hax] = plotcoulistexc(num, xlb, numlist, tint, yoffs, lb, ...
  CLIM_fac, logx)
%function [hax] = plotcoulistexc(num, xlb, numlist, tint, yoffs, lb, ...
%  CLIM_fac, logx)
...
for i=1:length(numlist)
  ...
  %
  exsig{numlist(i)} = exc.sig{numlist(i)}; 
...
exttlim  = [-matmax(abs(exttamp)) matmax(abs(exttamp))];
...
j=1;
axes('Position', ax{j});  hax(j) = gca;
[hyl hp] = subplottt(exc.tt*1e3, exc.sig{num}, xlb_tt, 'U_0 [V]', ...
    [0 2*tint], exttlim, fonts);
set(hp, 'color', 'r');
puttextonplot(gca, lx(j), ly(j), lb(j), 0, fonts);
