function [data,time,params]= readmdf_quick(file, tcnt)
%function [data,time,params]=lecmdf(file);
%script to read mdf files (transient recorder T112 for COURONNE)
%I got a lecmdf.m from JET-Systemtechnik, but modified it Feb. 2002
%Christiane Schr�der
%to be checked:
%Trigger-channel?
%not yet all cards included
%not yet SDF 
...
for ichannel=1:channel_saved
	...
	identifier_version=(setstr(fread(fid,16,'char')))';
    ...
    data_raw=fread(fid,tcnt,'int16');
    fseek(fid, 2.*(X_max-X_min+1) + curpos, 'bof');
    %--------
