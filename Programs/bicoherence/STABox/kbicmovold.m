...
for t=1:ntime
    ...
    ylabel('(b_{Tot}^{k})^{2}','fontsize',14)
    title('total k-bicoherence')
    ax=axis;
    ...
    subplot(122)
    hl1 = semilogy(k1(minind:maxind),sp(minind:maxind),'r','linewidth',2);
    axis([0 8 0.1*spmin 1.1*spmax]);
    ax1 = gca;
    ylabel('S(k) (a.u.)','fontsize',14);
    ...
    hold on
    plot(k1(minind:maxind),sb(minind:maxind),'.m','Parent',ax2);
    hl2 = plot(k1,se,':k','Parent',ax2);
    axis([0 8 0 2*max([sbmax semax])]);
    grid
    hold off
    xlabel('m','fontsize',14)
    ylabel('(b^{k}(a))^{2}','fontsize',14)
