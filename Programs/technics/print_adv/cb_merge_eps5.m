function [] = cb_merge_eps5(epsfile, jpegepsname, endepsfile_name, ...
  axnum, epsname, ax_cmpr, ax_same)
%==========================================================================
%function [] = cb_merge_eps5(epsfile, jpegepsname, endepsfile_name, ...
%  axnum, epsname, ax_cmpr)
%--------------------------------------------------------------------------
...
%           ax_cmpr vector showing the axes to compress
%           ax_same vector showing the axes being overlay plots
%              (not found as axis here)
...
%   plot(lines_PP, 1:length(lines_PP), 'gd')
% hold off
% figure
% hold on
%   plot(lines_stroke, ones(length(lines_stroke),1), 'b+')
%   plot(lines_6w,     ones(length(lines_6w),1),     'rs')
%   plot(lines_1sg,    ones(length(lines_1sg),1),    'ko')
%   plot(lines_0sg,    ones(length(lines_0sg),1),    'mx')
%   plot(lines_PP,     ones(length(lines_PP),1),     'gd')
% hold off
% --------------------------------------------------------- CHECK PLOTS ---
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Key pattern used to find each axis.
% Colorbar is found as an axis too.
...
for j=1:length(lines_PP)
  ...
  if k==length(str)-1
    ...
    k = strfind(str, 'stroke');
      if k==length(str)-5
        ctr = ctr + 1;
        lines_key(ctr) = lines_PP(j)+2;
      end
  end
end

if length(ax_cmpr) - sum(ax_same) ~= length(lines_key)
  disp('WARNING: length ax_cmpr not equal with lines_key (# axes)');
end


% begin with the biggest lines_key than the upper position stays the same
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while length(line_down) ~= 4
  str= text(lines_key(axnum)+ 1 + ctr).line;
  line_down= sscanf(str, '%d %d mt %d %d L');
  ctr = ctr+1;
end
ctr=ctr-1;

str= text(lines_key(axnum)+ 2 + ctr).line;
...
fid3 = fopen(endepsfile_name, 'wt');
  ...
  for i= lines_key(axnum): size(text, 2)
     fprintf(fid3, '%s\n', text(i).line );
  end
fclose(fid3);
